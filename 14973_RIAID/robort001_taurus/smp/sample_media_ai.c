...
#define Goods_FRM_WIDTH    640
#define Goods_FRM_HEIGHT   384
...
static HI_VOID GoodsClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (GetCfgBool("Goods_classify_switch:support_Goods_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = Yolo2GoodsDetectResnetClassifyLoad(&g_workPlug.model);
            if (ret < 0) {
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(ret < 0, Goods_RELEASE,
                    "load Goods classify model err, ret=%#x\n", ret);
            }
        }

        VIDEO_FRAME_INFO_S resizeFrm;
        ret = MppFrmResize(&frm, &resizeFrm, Goods_FRM_WIDTH, Goods_FRM_HEIGHT);
        ret = Yolo2GoodsDetectResnetClassifyCal(g_workPlug.model, &resizeFrm, &frm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, Goods_RELEASE,
            "Goods classify plug cal FAIL, ret=%#x\n", ret);

        ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
        SAMPLE_CHECK_EXPR_GOTO(ret != HI_SUCCESS, Goods_RELEASE,
...
static HI_VOID* GetVpssChnFrameGoodsClassify(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        GoodsClassifyAiProcess(frm, voLayer, voChn);
    }
...
static HI_S32 GoodsClassifyAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameGoodsClassify, NULL);
...
static HI_S32 PauseDoUnloadGoodsClassifyModel(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    if (GetCfgBool("Goods_classify_switch:support_Goods_classify", true)) {
         ...
         * When exiting the operation, the model should be unloaded
         */
        s32Ret = Yolo2GoodsDetectResnetClassifyUnload(g_workPlug.model);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload Goods model err:%x\n", s32Ret);
        ConfBaseExt();
        g_num = 0;
    }
 ...
 *
 * Display the data collected by the sensor on the LCD screen,
 * and create threads to run Goods detection and gesture classification reasoning calculations
 * VI->VPSS->VO->MIPI
 */
HI_S32 SAMPLE_MEDIA_Goods_CLASSIFY(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
     ...
     * Create work thread to run ai
     */
    s32Ret = GoodsClassifyAiThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));
    ...
    pthread_join(g_aiProcessThread, NULL);
    g_aiProcessThread = 0;
    PauseDoUnloadGoodsClassifyModel();
